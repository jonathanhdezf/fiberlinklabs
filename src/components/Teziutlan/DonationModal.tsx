...
const DonationModal = ({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) => {
    ...
    const handleDonation = () => {
        ...
        setIsLoading(true);
        // Simulating a redirect to a real payment gateway (Stripe/PayPal)
        // In a real environment, you would use the PayPal JS SDK or Stripe Checkout here.
        setTimeout(() => {
            setIsLoading(false);
            setStep('success');
        }, 3000);
    ...
    return (
        <div className="fixed inset-0 z-[250] flex items-center justify-center p-4">
            <div className="absolute inset-0 bg-stone-950/80 backdrop-blur-lg" onClick={step === 'success' ? undefined : onClose}></div>

            <div className="relative w-full max-w-lg bg-white dark:bg-stone-900 rounded-[2.5rem] overflow-hidden shadow-2xl border border-stone-200 dark:border-white/5 animate-in zoom-in-95 duration-300">
                {step !== 'success' && (
                    <div className="p-8 md:p-12">
                        <div className="flex justify-between items-start mb-8">
                            ...
                            <button onClick={onClose} className="p-2 hover:bg-stone-100 dark:hover:bg-white/5 rounded-full transition-colors">
                                    ...
                                    <div className="grid grid-cols-3 gap-3">
                                        ...
                                        <div className="relative col-span-1">
                                            <input
                                                type="number"
                                                placeholder="Otro"
                                                className="w-full h-full py-3 px-4 bg-stone-50 dark:bg-stone-800 border border-stone-200 dark:border-stone-700 rounded-2xl font-black text-stone-900 dark:text-white placeholder:text-stone-400 text-center outline-none focus:border-amber-500"
